...
- Frontend desktop fit and accessibility: global desktop typography scaling now adapts at tighter viewport widths and body overflow is scroll-safe to prevent clipping on Windows + WSL2 browsing scenarios that previously required extreme browser zoom.
- Frontend settings-panel reliability: workspace settings drawer now avoids low-height media-query clipping under high display scaling (for example 130%), preserving full-panel scrolling instead of limiting rendering to a partial area.
- Frontend shell interaction update: left-side navigation and session rails now run as a collapsible overlay drawer triggered from a left-edge button, so opening them no longer compresses the main conversation surface.
- Frontend shell polish: left-edge drawer trigger now uses a higher-contrast, vertically centered tab style, and the top navigation bar spacing is condensed for better fit on high-DPI displays.
- Frontend shell visibility pass: the left drawer trigger is now fixed to viewport center with stronger contrast and higher z-index, and top navigation controls are further compacted for a noticeably slimmer header footprint.
- Frontend chat-focus tuning: conversation top navigation now uses a denser compact layout (title, metadata, search, and controls) so the message area occupies more of the viewport.
- Frontend interaction polish: conversation subtitle metadata is now hidden by default and revealed on header hover/focus, and composer actions now place a compact Attach Files control and Send Message button on a single row.
- Frontend composer refresh: attachment and send actions are now integrated into the input row as compact icon controls (WhatsApp-like interaction), replacing the previous separate lower text-button row.
- Frontend composer density pass: attachment and send controls now sit together at the right edge of the input row, and composer spacing/textarea/icon sizing were tightened to reduce input-area viewport usage on high-DPI scaling (including 130% zoom scenarios).
- Frontend navigation interaction pass: the conversation top controls now use a top-center floating trigger that expands into an overlay navigation panel, matching the floating drawer interaction style while preserving settings/search/theme actions.
- Frontend top-nav follow-up fix: top navigation is now fully decoupled from legacy header layout classes so it stays collapsed by default and only appears when the top-center floating trigger is opened.
- Frontend composer anchoring fix: the chat composer no longer stretches as a tall grid area; it now keeps a compact height and stays aligned to the bottom edge of the conversation panel.
- Frontend layout root-cause fix: conversation panel composition moved from legacy multi-row grid to flex-column flow after top-nav floating changes, eliminating the large empty area that appeared below the composer.
- Frontend composer width fix: bottom composer alignment was updated to stretch across the full conversation row (instead of right-corner anchoring), matching a WhatsApp-like full-width input bar.
- Frontend top-nav usability fix: removed the blocking top-nav overlay layer and switched to click-outside detection, restoring full interactivity for search, scope, settings, and theme controls in the expanded top panel.
- Frontend floating-trigger stability fix: top-center and left-edge floating buttons now lock their positional transforms on hover/focus/active states so they no longer shift when pointer interactions apply shared button hover effects.
- Frontend hover behavior hardening: global ghost-button hover translation now explicitly excludes the top-center and left-edge floating triggers, so those controls use highlight-only hover feedback with no positional drift.
- Frontend composer resize control: added a top-edge drag handle on the chat composer so users can drag up/down to adjust visible input height on demand, while preserving a fixed minimum baseline height by default.
- Frontend composer resize interaction fix: drag-resize now uses pointer capture with precise height baselining from the live textarea box, removing cursor offset drift and ensuring resize release is reliable at minimum height so follow-up clicks remain responsive.
- Worksettings page refactor: clicking the top gear now opens a dedicated full-page worksettings view (instead of the previous side drawer), split into `Manage users` and `Analytics` tabs; user management includes create-invite, member removal, and member mute/unmute controls (mute state stored per workspace in local settings).
- Worksettings routing upgrade: the settings view now has a real route at `/worksettings` (via App Router page entry) and the gear/close interactions navigate between `/` and `/worksettings`, enabling native browser back/forward history behavior.
- Worksettings standalone-page mode: `/worksettings` now renders as a full dedicated settings surface that hides chat composer/timeline/side rails/top-nav overlays, so users see only settings and analytics content without chat background context.
- Theme hydration fix: route transitions to `/worksettings` now wait for initial theme readiness before writing theme state back to `documentElement` and localStorage, preventing accidental dark-mode resets when the current theme is light.
- Frontend profile settings update: the left drawer (opened from the left floating trigger) now includes display-name editing via `PATCH /auth/profile`, with immediate UI refresh for the signed-in identity label.
